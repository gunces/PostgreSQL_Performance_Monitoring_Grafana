...
SELECT  'bloats' || ',' || '$DB' || ',' || '$HOSTNAME' || ',' || '$HOSTNAME' || ',' || type || ',' || schemaname || ',' || object_name || ',' || bloat::float || ',' || (raw_waste) || ',' || object_size::float || ',' || '$UNIXTIME'
FROM
(SELECT
  'table' as type,
  schemaname,
